...
L4G
Sedėjau prie 10 kompiuterio
Pridėtos eilutės: 53,69,73,77,81,89,94,185-188,205-207
============================================================================
*/
...
void PrintResults(model (&printOut)[MAX_ARRAY_SIZE], double realResults[MAX_ARRAY_SIZE]);

__global__ void RunOnGPU(manufacturer *printOut, model *resultsArray, double *real);
...
int main() {
	...
	double realResults[MAX_ARRAY_SIZE];
	...
	PrintResults(results, realResults);
	system("Pause");
	return 0;
}
...
void PrintResults(model(&printOut)[MAX_ARRAY_SIZE], double realResults[MAX_ARRAY_SIZE]) {
	...
	cout << "------------------------------------------------------------------------------\n";
	for (int i = 0; i < MAX_ARRAY_SIZE; i++) {
		cout << "Realių sk. masyvo " << i + 1 << " elementas: " << realResults[i] << endl;
	}
}
...
__global__ void RunOnGPU(manufacturer *printOut, model *resultsArray, double *real) {
	...
	for (int i = 0; i < MAX_THREADS; i++) {
		...
		if (i == 0) {
			real[gija] = printOut[i].models[gija].price - printOut[i].models[gija].quantity;
		}
	}
}
