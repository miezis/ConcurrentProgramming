...
@process
def manager(prod_cnt, cons_cnt, logger, prodList, consList, takeList, buffer):
    consumersCount = [cons_cnt]
    producersCount = [prod_cnt]
    ...
    while consumersCount[0] > 0 or producersCount[0] > 0:
        FairSelect( # alternatyvos pasirinkimas
            InputGuard(prodList, action=add(producersCount)),
            InputGuard(consList, action=take(consumersCount, producersCount, takeList))
        )
...
def main():
    ...
    producerMessages = Channel('Producers', buffer=len(manufacturers))
    consumerMessages = Channel('Consumers', buffer=len(counters))
    taken = Channel('Taken', buffer=len(counters))
    logger = Channel('log')
    ...
    Spawn(
        ...
        manager(
            ...
            len(counters),
            logger.writer(),
            producerMessages.reader(),
            consumerMessages.reader(),
            taken.writer(),
            buffer
        ),
        [producer(producerMessages.writer(), manufacturers[id]) for id in xrange(len(manufacturers))],
        [consumer(consumerMessages.writer(), taken.reader(), counters[id]) for id in xrange(len(counters))]
    )
